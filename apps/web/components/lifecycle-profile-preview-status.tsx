...
export function LifecycleProfilePreviewStatus(): React.JSX.Element {
  const [events, setEvents] = useState<BufferedEvent[]>([])

  useEffect(() => {
    const refresh = () => {
      setEvents((window.__DRAPEON_ANALYTICS_BUFFER__ ?? []).map((event) => ({
        name: event.name,
  ...
  return (
    <section className="mt-6 rounded-[8px] border border-needle/20 bg-needle/5 p-4" data-testid="lifecycle-profile-preview-status">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-needle">Emitted preview events</p>
      {events.length === 0 ? (
        <p className="mt-2 text-sm text-ink/60">No consented profile event yet.</p>
      ) : (
